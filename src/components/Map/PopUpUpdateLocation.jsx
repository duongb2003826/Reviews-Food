...
const PopUpUpdateLocation = ( { isShowCreate, setIsShowCreate, user, item, ...props } ) =>
{
	...
	const [ menu, setMenu ] = useState( item.menus || [
		{
			nameFood: "",
			description: "",
			linkFood: "",
	...
	useEffect( () =>
		{
			if ( item )
			{
				let category = props.categories?.find( item => item._id == item.category_id );
				let key = category?.name == 'Đồ ăn' && 'FOOD' || 'DRINK';
				let sub = SUB_CATE.filter( item => item.type == key );
				if(sub?.length > 0) {
					sub = sub.map(e => {
						console.log(item.sub_category, e.code)
						e.checked = item.sub_category?.includes(e.code) || false;
						return e
					});
				}
				setSubcategories( sub || [] )
		...
		// 		.filter(item => item.checked) // Lọc ra các phần tử đã được chọn
		// 		.map(item => item.name); // Chỉ lấy tên của các phần tử đã được chọn
		// 	return selectedSubCategories.join(','); // Trả về một chuỗi các tên phân loại đã được chọn, ngăn cách bởi dấu phẩy
		// };
...
export default PopUpUpdateLocation;
